...
public class LootingBag {
    ...
    public void depositAllLootBag() {
        ...
        if (!player.Area(Config.HOME_X1, Config.HOME_X2, Config.HOME_Y1, Config.HOME_Y2) && !player.inBank() || player.inWild()) {
    ...
    public void sendInventoryItems() {
        ...
        final int START_ITEM_INTERFACE = 39448; 
        for (int i = 0; i < 28; i++) {
            ...
            if(id == 11942) {
            	player.getPA().sendFrame34a(START_ITEM_INTERFACE + i, -1, 0, amt);
            } else {
            	player.getPA().sendFrame34a(START_ITEM_INTERFACE + i, id - 1, 0, amt);
            }
            //player.sendMessage("Sent item: " + i + " id: " + id);
        }
    }
