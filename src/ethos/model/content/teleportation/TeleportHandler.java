...
public class TeleportHandler {
	...
	public void handleTeleports(Player player, int Id) {
		...
		case 161033: // Monsters Tab
			selection(player, 0);
			break;
		case 161034: // Minigames Tab
			selection(player, 1);
			break;
		case 161036: // Bosses Tab
			selection(player, 2);
			break;
		case 161035: // Wilderness Tab
			selection(player, 3);
			break;
		case 161038: // City Tab
			selection(player, 4);
			break;
		case 161039: // Donator Tab
			// if (player.getRights().isDonator() || player.getRights().isSuperDonator() ||
			// player.getRights().isLegendaryDonator() ||
			// player.getRights().isExtremeDonator() ||
			// c.getRights().isOrInherits(Right.OWNER))
			...
			// player.sendMessage("You must be a donator to use this.");
			break;
		case 161037: // Other Tab
			selection(player, 6);
			break;
		// End of Tabs

		// Start of Buttons
		case 180184: // Button 1
			...
			if (player.teleSelected == 0) { // Monsters - Rock Crabs
				...
				player.getPA().startTeleport(3122, 3481, 0, "modern", false); // change
				// player.sendMessage("Teleporting to "+otherNames[1]+".");
			}
			break;
		case 180185: // Button 2
			if (!teleportCheck(player))
				return;
