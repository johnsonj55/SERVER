...
public class ItemAssistant {
	...
	public int[][] skillcapes = { { 9747, 9748 }, // Attack
			...
			{ 9810, 9811 }, // Farming
			{ 9765, 9766 }, // Runecraft
			{ 33033 }, // master agility
			{ 33034 }, // master attack
			{ 33035 }, // MASTER_CONSTRUCTION
			{ 33036 }, // MASTER_COOKING
			{ 33037 }, // MASTER_CRAFTING
			{ 33038 }, // MASTER_DEFENCE
			{ 33039 }, // MASTER_FARMING
			{ 33040 }, // MASTER_FIREMAKING
			{ 33041 }, // MASTER_FISHING
			{ 33042 }, // MASTER_FLETCHING
			{ 33043 }, // MASTER_HERBLORE
			{ 33044 }, // MASTER_HITPOINTS
			{ 33045 }, // MASTER_HUNTER
			{ 33046 }, // MASTER_MAGIC
			{ 33047 }, // MASTER_MINING
			{ 33048 }, // MASTER_PRAYER
			{ 33049 }, // MASTER_RANGE
			{ 33050 }, // MASTER_RUNECRAFTING
			{ 33051 }, // MASTER_SLAYER
			{ 33052 }, // MASTER_SMITHING
			{ 33053 }, // MASTER_STRENGTH
			{ 33054 }, // MASTER_THIEVING
			{ 33055 } // MASTER_WOODCUTTING
	...
	public void addSpecialBar(int weapon) {
		...
		case 12904: // Toxic Staff
		case 20784: // Dragon claws fixed
		...
		if (c.getOutStream() != null && c != null) {
			if (c.playerEquipment[slot] > -1) {
				if (addItem(c.playerEquipment[slot], c.playerEquipmentN[slot])) {
					...
					if (c.playerEquipment[slot] == 30626) {
						if (c.playerStandIndex != 1501) {
							c.startAnimation(1500);
							c.playerStandIndex = 1501;
							c.playerTurnIndex = 1851;
	...
	public void resetBank() {
		...
		c.getPA().sendFrame36(1111, 0);
		c.getPA().sendFrame34a(58040, -1, 0, 0);
		int newSlot = -1;
		for (int i = 0; i < c.getBank().getBankTab().length; i++) {
			BankTab tab = c.getBank().getBankTab()[i];
			if (i == tabId) {
				c.getPA().sendFrame36(1111 + i, 1);
			} else {
				c.getPA().sendFrame36(1111 + i, 0);
			}
	...
	public int getPlatinumAmount(int ItemID) {
		int itemCount = 0;
		for (int i = 0; i < c.playerItems.length; i++) {
			if ((c.playerItems[i] - 1) == ItemID) {
				itemCount += c.playerItemsN[i];
	...
	/*
	public boolean royalSeedClick(Player c, int id) {
		if (id == 19564) {
...
*/
	...
	public void handleAmethyst() {
		if (!c.boltTips&&!c.arrowTips&&!c.javelinHeads) {
			...
			c.getPA().addSkillXP((c.getMode().getType().equals(ModeType.OSRS) ? 140 : 220), 12, true);
	...
	public void updateEquipment(int idx) {
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrameVarSizeWord(34);
			c.getOutStream().writeWord(1688);
			c.getOutStream().writeByte(idx);
			c.getOutStream().writeWord(c.playerEquipment[idx] + 1);

			if (c.playerEquipmentN[idx] > 254) {
				c.getOutStream().writeByte(255);
				c.getOutStream().writeDWord(c.playerEquipmentN[idx]);
			} else {
				c.getOutStream().writeByte(c.playerEquipmentN[idx]);
			}
			c.getOutStream().endFrameVarSizeWord();
			c.flushOutStream();
		}
