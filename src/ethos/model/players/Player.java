...
import ethos.model.content.CooksAssistant;
import ethos.model.content.DoricsQuest;
import ethos.model.content.DragonSlayer2;
import ethos.model.content.MonkeyMadness;
import ethos.model.content.MurderMystery;
...
//import ethos.model.entity.npcs.konar.hydra.AlchemicalHydraConstants;
//import ethos.model.entity.npcs.konar.hydra.AlchemicalHydraRedone;
...
import ethos.model.content.bonus.SkillOfTheDay;
import ethos.model.content.collection.CollectionLogHandler;
import ethos.model.items.bank.BankPin;
import ethos.model.minigames.CastleWars;
...
import ethos.model.npcs.bosses.Nightmare;
...
import ethos.model.players.packets.commands.all.Donate;
...
import ethos.model.content.music.PlayList;
import ethos.model.content.preset.PresetManager;
import ethos.net.ActionSender;

public class Player extends Entity {
	
	public int questTab = 0;
	public boolean augury;
	public boolean rigour;
	public int imbuedSlayerHelmKills[] = {0, 0, 0, 0};
	...
	public int privateBounty = -1;
	public boolean hasReached5Killstreak = false;
	public boolean hasReached6Killstreak = false;
	public boolean hasReached7Killstreak = false;
	public boolean hasReached8Killstreak = false;
	public boolean hasReached9Killstreak = false;
	public boolean hasReached10Killstreak = false;
	public MageArena getMageArena() {
		return this.mageArena;
	}
	public int[][] raidReward ={{0,0}};
	public int raidCount;
	public boolean PK_RESTRICTED;
	...
	public void resetInfernoInstance() {
		this.inferno = null;
	}

	public Tzkalzuk getInfernoInstance() {
		return tzkalzuk;
	}
	/**
	...
	public int item, uneditItem, quantity, price, pageId = 1;
	public Listing.Category searchCategory;
	...
	private SlayerChest slayerChest= new SlayerChest();
	private UltraMysteryBox ultraMysteryBox= new UltraMysteryBox(this);
	private RandomPetBox randomPetBox= new RandomPetBox(this);

	public UltraMysteryBox getUltraMysteryBox() {
		return ultraMysteryBox;
	}
	public SlayerChest getSlayerChest() {
		return slayerChest;
	}
	public RandomPetBox getRandomPetBox() {
		return randomPetBox;
	}
	...
	private DoricsQuest doricsQuest;
	private CooksAssistant cooksAssistant;
	private MurderMystery murderMystery;
	private MonkeyMadness monkeyMadness;
	private DragonSlayer2 dragonSlayer2;
...
//	private AlchemicalHydraRedone hydra = null;
	private Tzkalzuk tzkalzuk = null;
	private Skotizo skotizo = null;
	private Nightmare nightmare = null;
	...
	public RandomLoot getRandomLoot() {
		return randomLoot;
	}
	public boolean claimedPurePackage = false;
	public int raidPoints, unfPotHerb, cwKills, cwDeaths, cwGames, unfPotAmount, wrenchObject = 0, halloweenOrderNumber = 0, speed1 = -1,
			...
			playerRunIndex = 0x338, fletchingType,
	...
	private int chatTextColor = 0, chatTextEffects = 0, dragonfireShieldCharge, runEnergy = 100, lastEnergyRecovery,
			...
			toxicTridentCharge, sangStaffCharge, arcLightCharge, etherCharge, crawCharge, vigCharge, sceptreCharge,
			runningDistanceTravelled, trivaPoints, interfaceOpen;
	...
	public boolean talkAnna = false, talkGossip = false, talkStanford = false, talkMary = false, talkLouisa = false,
			talkHobbes = false, talkPierre = false, talkDonovan = false, talkBob = false, talkCarol = false,
			talkDavid = false, talkElizabeth = false, talkFrank = false;
	...
	public boolean teleportingToDistrict = false, morphed = false, isIdle = false, boneOnAltar = false,
			...
			isMoving, walkingToItem, isShopping, updateShop, update10, forcedChatUpdateRequired, inDuel, tradeAccepted, goodTrade,
	...
	public void highscores() {
		getPA().sendFrame126("Mistex - Top PKers Online", 6399); // Title
		for (int i = 0; i < 10; i++) {
			if (ranks[i] > 0) {
	...
	public void destruct() {
		...
		if(getMode().isIronman()) {
			com.everythingrs.hiscores.Hiscores.update("T39x6MUzQWCf2dm7LCPMlhH25yRExxWMcUFN5zgOASDvrjeMWn8XrwXSTSgjcozNDc39D454",  "Iron Man", this.playerName, this.playerRank, this.playerXP, debugMessage);
		}
		if(getMode().isUltimateIronman()) {
			com.everythingrs.hiscores.Hiscores.update("T39x6MUzQWCf2dm7LCPMlhH25yRExxWMcUFN5zgOASDvrjeMWn8XrwXSTSgjcozNDc39D454",  "Ultimate Iron Man", this.playerName, this.playerRank, this.playerXP, debugMessage);
		}
		if(getMode().isRegular()) {
			com.everythingrs.hiscores.Hiscores.update("T39x6MUzQWCf2dm7LCPMlhH25yRExxWMcUFN5zgOASDvrjeMWn8XrwXSTSgjcozNDc39D454",  "Normal Mode", this.playerName, this.playerRank, this.playerXP, debugMessage);
		}
	...
	public void initialize() {
		try {
			 ...
			 * Welcome messages
			 */
			sendMessage("Welcome to <col=191970>Mistex!");
			sendMessage("@dre@Update: The Double Exp Is Acitve and Various Bug Fixes!");
			sendMessage("@dre@Check out the new content!");
			if (getSlayer().superiorSpawned) {
				getSlayer().superiorSpawned = false;
			}
			SkillOfTheDay.skillOfTheDayMessage(this);
			// checkWellOfGoodwillTimers();

			if (getName().equalsIgnoreCase("Josh")) {
			...
			TeleportTab.updateInterface(this, TeleportTab.Category.MONSTERS);
	...
	public void updateQuestTab(){
		questTab = 0;
		getPA().sendFrame126("<img=29> @yel@Server data",10409);
		getPA().sendFrame126("@bla@-@or1@ Players online : @whi@"+ PlayerHandler.getPlayerCount(),10410);
		getPA().sendFrame126("@bla@-@or1@ Wilderness Players: @whi@"+Boundary.entitiesInArea(Boundary.WILDERNESS),10411);
		getPA().sendFrame126("", 10412);
		getPA().sendFrame126("<img=32> @yel@Misc data", 10413);
		...
		getPA().sendFrame126("@bla@-@or1@ Time Played = @whi@"+time,10414);
		getPA().sendFrame126("@bla@-@or1@ Player Rank = @whi@"+getRights().getPrimary().toString(),10415);
		getPA().sendFrame126("@bla@-@or1@ Amount Donated: @whi@" + this.donatorPoints, 10416);
		String dailyStatus = "";
		if(this.dailyEnabled) {
			if(this.completedDailyTask)
				dailyStatus = "@gre@Completed";
			else 
				dailyStatus = "@yel@Incomplete";
		} else 
			dailyStatus = "@whi@Disabled";
		getPA().sendFrame126("@bla@-@or1@ Daily Task: " + dailyStatus,10417);
		getPA().sendFrame126("", 10418);
		getPA().sendFrame126("<img=16> @yel@Your Points", 10419);
		getPA().sendFrame126("@bla@-@or1@Player points:", 10420);
		getPA().sendFrame126("@bla@-@or1@ Vote Points: @whi@" + this.votePoints, 10421);
		getPA().sendFrame126("@bla@-@or1@ PK Points = @whi@" + this.pkp, 10422);
		getPA().sendFrame126("@bla@-@or1@ Slayer Points = @whi@" + this.getSlayer().getPoints(), 10423); //m.getPointReward(0)
		getPA().sendFrame126("@bla@-@or1@ PC points = @whi@" + this.pcPoints, 10424);
		getPA().sendFrame126("@bla@-@or1@ Shayzien points = @whi@" +this.shayPoints, 10425);
		getPA().sendFrame126("@bla@-@or1@ Raids Points = @whi@" + this.raidPoints, 10426);
	}
	
	public void updateAcheivementTab() {
		questTab = 1;
		int diariesComplete = this.amountOfDiariesComplete();
		
		getPA().sendFrame126(" @or1@               Achievement Diary",10407);
		getPA().sendFrame126(" @or1@Varrock", 10408);
		getPA().sendFrame126(" @or1@Ardougne", 10409);
		getPA().sendFrame126(" @or1@Desert", 10410);
		getPA().sendFrame126(" @or1@Falador", 10411);
		getPA().sendFrame126(" @or1@Fremnnik", 10412);
		getPA().sendFrame126(" @or1@Kandarin", 10413);
		getPA().sendFrame126(" @or1@Karamja", 10414);
		getPA().sendFrame126(" @or1@Lumbridge & Draynor", 10415);
		getPA().sendFrame126(" @or1@Morytania", 10416);
		getPA().sendFrame126(" @or1@Western", 10417);
		getPA().sendFrame126(" @or1@Wilderness", 10418);
		getPA().sendFrame126(" @or1@Diaries Completed: @gre@" + diariesComplete, 10419);
		getPA().sendFrame126("", 10420);
		getPA().sendFrame126("", 10421);
		getPA().sendFrame126("", 10422);
		getPA().sendFrame126("", 10423);
		getPA().sendFrame126("", 10424);
		getPA().sendFrame126("", 10425);
		getPA().sendFrame126("", 10426);
		getPA().sendFrame126("", 10427);
		getPA().sendFrame126("", 10428);
		getPA().sendFrame126("", 10429);
		getPA().sendFrame126("", 10430);
		getPA().sendFrame126("", 10431);
		getPA().sendFrame126("", 10432);
		getPA().sendFrame126("", 10433);
		getPA().sendFrame126("", 10434);
		getPA().sendFrame126("", 10435);
		getPA().sendFrame126("", 10436);
		getPA().sendFrame126("", 10437);
		getPA().sendFrame126("", 10438);
		getPA().sendFrame126("", 10439);
		getPA().sendFrame126("", 10440);

	}

	public void updateRedTab() {
		questTab = 2;
		getPA().sendFrame126("@or1@Quick actions:", 10409);
		getPA().sendFrame126("@or1@ Open Achievements",10410);
		getPA().sendFrame126("@or1@ Preload Gear", 10411);
		getPA().sendFrame126("@or1@ Collection log", 10412);
		getPA().sendFrame126("@or1@ Open NPC Droptable",10413);
		getPA().sendFrame126("",10414);
		getPA().sendFrame126("<img=22> @yel@Pk data", 10415);
		getPA().sendFrame126("@bla@-@or1@Kills: @whi@" + this.killcount, 10416);
		getPA().sendFrame126("@bla@-@or1@Death: @whi@" + this.deathcount, 10417);
		getPA().sendFrame126("@bla@-@or1@ My KDR: @whi@"+ (double)(this.deathcount == 0 ? this.killcount + this.deathcount : this.killcount/this.deathcount), 10418);
		getPA().sendFrame126("@bla@-@or1@ PK Restricted: @whi@" + this.PK_RESTRICTED, 10419);
		getPA().sendFrame126("", 10420);
		getPA().sendFrame126("", 10421);
		getPA().sendFrame126("", 10422);
		getPA().sendFrame126("", 10423);
	...
	public void updatePurpleTab() {
		questTab = 3;

		getPA().sendFrame126("@or1@ Quests:", 10409);
		getPA().sendFrame126(" Cook's Assistant", 10410);
		/*
		if (c.getCooksAssistant().getStage("CooksAssistant") == 0) {
			sendFrame126("@red@Cook's Assistant", 10410);
		}
		if (c.getCooksAssistant().getStage("CooksAssistant") == 1) {
			sendFrame126("@yel@Cook's Assistant", 10410);
		}
		if (c.getCooksAssistant().getStage("CooksAssistant") == 2) {
			sendFrame126("@gre@Cook's Assistant", 10410);
		}
		*/
		getPA().sendFrame126(" Doric's Quest", 10411);
		getPA().sendFrame126(" Monkey Madness", 10412);
		getPA().sendFrame126(" Murder Mystery", 10413);
		getPA().sendFrame126("", 10414);
		getPA().sendFrame126("", 10415);
		getPA().sendFrame126("", 10416);
		getPA().sendFrame126("", 10417);
		getPA().sendFrame126("", 10418);
		getPA().sendFrame126("", 10419);
		getPA().sendFrame126("", 10420);
		getPA().sendFrame126("", 10421);
		getPA().sendFrame126("", 10422);
		getPA().sendFrame126("", 10423);
	...
	public void logout() {
		...
		IronmanTeamHandler.handleLogout(this); //forgot this before that's why you got the error
	...
	public int[] SLAYER_HELMETS = { 11864, 11865, 19639, 19641, 19643, 19645, 19647, 19649,21888, 749, 751, 752 };
	public int[] IMBUED_SLAYER_HELMETS = { 11865, 19641, 19645, 19649, 749, 751, 752 };
	...
	public void process() {
		...
		if (Server.getMultiplayerSessionListener().inAnySession(this) && alreadyChecked == false &&  (this.followId != -1 || this.followId != -2)) {
			playerAssistant.resetFollow(); 
			getTrade().request(cliento2);
			if (speed > 0) {
				speed--;
			} else {
				alreadyChecked = true;
		...
		if (this.playTime % 3600 == 0 && !isIdle && !trading){
			if (getHourlyBoxToggle() == true) {
				...
				sendMessage("[ @red@Reward @bla@] <img=1>@blu@Thank you for continuing to play Mistex, here is an @red@Hourly Reward Box@bla@!");
			} else {
		...
		Optional<IronmanTeam> getTeam = this.getIronmanTeam();

		if(getTeam.isPresent()) {
			if (getMode().isGroupIronman()
					&& getTeam.get().leaderName.equalsIgnoreCase(playerName)
					&& !getTeam.get().isTeamFull()) {
	...
	public Optional<IronmanTeam> getIronmanTeam() {
		return IronmanTeamHandler.getPlayersTeam(this);
	}
	...
	public Kraken createKrakenInstance() {
		Boundary boundary = Boundary.KRAKEN_CAVE;
	...
	/*
	public AlchemicalHydraRedone createAlchemicalHydraInstance() {
		Boundary boundary = AlchemicalHydraConstants.AREA;

		int instanceHeight = InstancedAreaManager.getSingleton().getNextOpenHeight(boundary);

		this.hydra = new AlchemicalHydraRedone(this, boundary, instanceHeight);
	...
	public CooksAssistant getCooksAssistant() {
		if (this.cooksAssistant == null) {
			this.cooksAssistant = new CooksAssistant();
		}
		return this.cooksAssistant;
	}

	public DoricsQuest getDoricsQuest() {
		if (this.doricsQuest == null) {
			this.doricsQuest = new DoricsQuest();
		}
		return this.doricsQuest;
	}

	public MurderMystery getMurderMystery() {
		if (this.murderMystery == null) {
			this.murderMystery = new MurderMystery();
		}
		return this.murderMystery;
	}

	public MonkeyMadness getMonkeyMadness() {
		if (this.monkeyMadness == null) {
			this.monkeyMadness = new MonkeyMadness();
		}
		return this.monkeyMadness;
	}

	public DragonSlayer2 getDragonSlayer2() {
		if (this.dragonSlayer2 == null) {
			this.dragonSlayer2 = new DragonSlayer2();
		}
		return this.dragonSlayer2;
	}
	...
	public final int[] BOWS = { 19481, 19478, 12788, 9185, 11785, 21012, 21902, 839, 845, 847, 851, 855, 859, 841, 843, 849,
	...
	public boolean inWild() {
		...
		if (Boundary.isIn(this, Boundary.REV_CAVE))
			return true;
	...
	public boolean inRevs(Player c) {
		return (Boundary.isIn(c, Boundary.REV_CAVE));
		//return (absX > 3143 && absX < 3262 && absY > 10053 && absY < 10231);
	}

	public boolean inMulti() {
		if (Boundary.isIn(this, Zulrah.BOUNDARY)||(Boundary.isIn(this, Kraken.BOUNDARY)) || Boundary.isIn(this, Boundary.CORPOREAL_BEAST_LAIR)
				...
				|| Boundary.isIn(this, Boundary.ICE_DEMON) || Boundary.isIn(this, Boundary.REV_CAVE) || Boundary.isIn(this, Boundary.CATACOMBS)
...
//	public int playerSmithing = 13;
	...
	public boolean absorptionToggle = false;
	...
	public int i;
	public int playerId = -1; // i added this for castle wars
	public PresetManager presetManager = new PresetManager(this);
	public CollectionLogHandler collectionLog = new CollectionLogHandler(this);
	...
	public int getEtherCharge() {
		return etherCharge;
	}
	
	public void setEtherCharge(int chargeEther) {
		this.etherCharge = chargeEther;
	}
	...
	public int getVigCharge() {
		return vigCharge;
	}
	
	public void setVigCharge(int chargeVig) {
		this.vigCharge = chargeVig;
	}
	public int getSceptreCharge() {
		return sceptreCharge;
	}
	
	public void setSceptreCharge(int chargeSceptre) {
		this.sceptreCharge = chargeSceptre;
	}
	...
	/*
	public AlchemicalHydraRedone getHydra() {
		return hydra;
	}
	*/
	public void setQuick(boolean[] prayers) {
		quick.normal = Arrays.copyOf(prayers, prayers.length);
	}
