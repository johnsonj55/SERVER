...
public class PlayerAssistant {
	...
	private boolean hasItemsToReclaim;
	...
	public void sendItems(int componentId, List<GameItem> items) {
		if (c == null || c.getOutStream() == null) {
			return;
		}
	...
	public void sendItems(Player player, int componentId, int[] items, int[] amounts, int capacity, boolean increaseId) {
		if (player == null || player.getOutStream() == null) {
			return;
		}
		player.getOutStream().createFrameVarSizeWord(53);
		...
		for (int idx = 0; idx < length; idx++) {
			if (amounts[idx] > 254) {
				player.getOutStream().writeByte(255);
				player.getOutStream().writeDWord_v2(amounts[idx]);
			} else {
				player.getOutStream().writeByte(amounts[idx]);
			}
			player.getOutStream().writeWordBigEndianA(items[idx] + (increaseId ? 1 : 0));
			current++;
		}
		for (; current < capacity; current++) {
			player.getOutStream().writeByte(1);
	...
	public void loginScreen() {
		showInterface(15244);
		sendFrame126("Welcome to " + Config.SERVER_NAME + "\\n", 15257);
		   int currentDay = c.getLastLogin() - c.lastLoginDate;
	        if (c.lastLoginDate <= 0) {
	            sendFrame126("This is your first time logging in!", 15258);
	        } else if (c.lastLoginDate == 1) {
		...
		sendFrame126("Remember to vote daily to help " + Config.SERVER_NAME + "", 15803);
...
*/
	...
	public void updateProgressBar(int id, byte percentage, String text) {
		if (c != null && c.getOutStream() != null) {
			c.getOutStream().createFrameVarSizeWord(77);
			c.getOutStream().writeWordA(id);
			c.getOutStream().writeByte(percentage);
	...
	public void castVengeance() {
		...
		c.gfx100(726);
		addSkillXP(c.getMode().isOsrs() ? 448 : 10000, 6, true);
		refreshSkill(6);
		c.vengOn = true;
		c.usingMagic = false;
	...
	public void alchemy(int itemId, String alch) {
		...
		case "low":
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				...
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[49][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : 31), 6,
						true);
				refreshSkill(6);
			}
			break;
		...
		case "high":
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				...
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[50][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : 65), 6,
						true);
				refreshSkill(6);
			}
			break;
						...
						if (c.inWild()) {
							...
							if (!playerKiller.getPlayerKills().killedRecently(c.connectedFrom)
								...
								if (Boundary.isIn(c, Boundary.SAFEPK)) {
									...
									if (c.privateBounty > -1) {
										playerKiller.getItems().addItemUnderAnyCircumstance(995, c.privateBounty);
										playerKiller.getPA().yell(playerKiller.getName() + " has slayed " + c.getName() + " for " + c.privateBounty + " coins");
										c.privateBounty = -1;  
									}
									if(bonuspkp > 0) {
	...
	public void giveLife() {
		...
		//A question to you: do you want the players to be able to lose items while dieing in vorkath or not?not. So they keep their itemsyea alright
		// That should work, if not then contact me.alright. we will test it anyway
		if (!CastleWars.isInCw(c) && !c.inDuelArena() && !Boundary.isIn(c, Boundary.DUEL_ARENA)&& c.getVorkath().getVorkathInstance() == null
	...
	public boolean canTeleport(String type) {
		...
		if (c.inWild()) {
			if (!type.equals("glory") && !type.equals("seedpod")) {
	...
	public void startTeleport(int x, int y, int height, String teleportType, boolean homeTeleport) {
		...
		if (c.inWild() && !(c.getRights().isOrInherits(Right.MODERATOR))) {
			if (!teleportType.equals("glory") && !teleportType.equals("seedpod") && !teleportType.equals("obelisk")) {
				if (c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
					c.sendMessage(
							"You can't teleport above level " + Config.NO_TELEPORT_WILD_LEVEL + " in the wilderness.");
					c.getPA().closeAllWindows();
	...
	public void followNpc() {
		...
		if (!projectile
			...
			if (c.playerEquipment[c.playerWeapon] == 11907
					|| c.playerEquipment[c.playerWeapon] == 12899 || c.playerEquipment[c.playerWeapon] == 22516 && bowDistance && !sameSpot) {
				return;
			}
	...
	public void logGamble() {
		String filePath = "./Data/gamblelogs.txt";
		BufferedWriter bw = null;
	...
	public void objectAnim(int X, int Y, int animationID, int tileObjectType, int orientation) {
		for (Player p : PlayerHandler.players) {
			if (p != null) {
				if (p.distanceToPoint(X, Y) <= 25) {
					p.getPA().createPlayersObjectAnim(p, X, Y, animationID, tileObjectType, orientation);
				}
			}
		}
	}
	
	public void createPlayersObjectAnim(Player player, int X, int Y, int animationID,
...
//			player.getOutStream().writeByteC(X - (player.mapRegionX * 8));
	...
	public boolean hasItemsToReclaim() {
		return hasItemsToReclaim;
	}
	
	public void setHasItemsToReclaim(boolean hasItemsToReclaim) {
		this.hasItemsToReclaim = hasItemsToReclaim;
	}
