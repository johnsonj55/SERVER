...
import ethos.model.content.preset.PresetClass;
...
public class PlayerSave {
		...
		while (EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					...
					} else if (token.equals("pkrestricted")) {
						p.PK_RESTRICTED = Boolean.parseBoolean(token2);
					} else if (token.equals("claimedpure")) {
						p.claimedPurePackage = Boolean.parseBoolean(token2);
					...
					} else if (token.equals("preset-title")) {
						p.presetManager.presets[Integer.parseInt(token3[0])].title = token3[1];
					} else if (token.equals("kill-log-amount")) {
						p.collectionLog.kill_logs.put(token3[0], new CollectionLog());
						p.collectionLog.kill_logs.get(token3[0]).amount = Integer.parseInt(token3[1]); 
					} else if (token.equals("kill-log-inventory")) {
						for (int idx = 1; idx < token3.length-1; idx++)
							p.collectionLog.kill_logs.get(token3[0]).addLoot(Integer.parseInt(token3[idx]), Integer.parseInt(token3[++idx])); 
					} else if (token.equals("mini-log-amount")) {
						p.collectionLog.minigame_logs.put(token3[0], new CollectionLog());
						p.collectionLog.minigame_logs.get(token3[0]).amount = Integer.parseInt(token3[1]); 
					} else if (token.equals("mini-log-inventory")) {
						for (int idx = 1; idx < token3.length-1; idx++)
							p.collectionLog.minigame_logs.get(token3[0]).addLoot(Integer.parseInt(token3[idx]), Integer.parseInt(token3[++idx])); 
					} else if (token.equals("clue-log-amount")) {
						p.collectionLog.clue_logs.put(token3[0], new CollectionLog());
						p.collectionLog.clue_logs.get(token3[0]).amount = Integer.parseInt(token3[1]); 
					} else if (token.equals("clue-log-inventory")) {
						for (int idx = 1; idx < token3.length-1; idx++)
							p.collectionLog.clue_logs.get(token3[0]).addLoot(Integer.parseInt(token3[idx]), Integer.parseInt(token3[++idx])); 
					} else if (token.equals("other-log-amount")) {
						p.collectionLog.other_logs.put(token3[0], new CollectionLog());
						p.collectionLog.other_logs.get(token3[0]).amount = Integer.parseInt(token3[1]); 
					} else if (token.equals("other-log-inventory")) {
						for (int idx = 1; idx < token3.length-1; idx++)
							p.collectionLog.other_logs.get(token3[0]).addLoot(Integer.parseInt(token3[idx]), Integer.parseInt(token3[++idx])); 
					} else if (token.equals("preset-spellbook")) {
						p.presetManager.presets[Integer.parseInt(token3[0])].spellbook = token3[1];
					
					} else if (token.equals("preset-spellbook")) {
						p.presetManager.presets[Integer.parseInt(token3[0])].spellbook = token3[1];
					} else if (token.equals("preset-inventory")) {
						PresetClass preset = p.presetManager.presets[Integer.parseInt(token3[0])];
						int iIdx = 0, iIdx2 = 0;
						for (int idx = 1; idx < token3.length-1; idx++) {
							preset.inventory[iIdx++] = Integer.parseInt(token3[idx]);
							preset.inventoryN[iIdx2++] = Integer.parseInt(token3[++idx]);
						}
					} else if (token.equals("preset-equipment")) {
						PresetClass preset = p.presetManager.presets[Integer.parseInt(token3[0])];
						int eIdx = 0, eIdx2 = 0;
						for (int idx = 1; idx < token3.length-1; idx++) {
							preset.equipment[eIdx++] = Integer.parseInt(token3[idx]);
							preset.equipmentN[eIdx2++] = Integer.parseInt(token3[++idx]);
						}
					} else if (token.equals("preset-prayers")) {
						PresetClass preset = p.presetManager.presets[Integer.parseInt(token3[0])];
						for (int idx = 1; idx < token3.length; idx++) {
							preset.quickprayers[idx-1] = Boolean.parseBoolean(token3[idx]);
						}
					...
					} else if (token.equals("privatebounty")) {
						p.privateBounty = Integer.parseInt(token2);
					} else if (token.equals("hasreached5killstreak")) {
						p.hasReached5Killstreak = Boolean.parseBoolean(token2);
					} else if (token.equals("hasreached6killstreak")) {
						p.hasReached6Killstreak = Boolean.parseBoolean(token2);
					} else if (token.equals("hasreached7killstreak")) {
						p.hasReached7Killstreak = Boolean.parseBoolean(token2);
					} else if (token.equals("hasreached8killstreak")) {
						p.hasReached8Killstreak = Boolean.parseBoolean(token2);
					} else if (token.equals("hasreached9killstreak")) {
						p.hasReached9Killstreak = Boolean.parseBoolean(token2);
					} else if (token.equals("hasreached10killstreak")) {
						p.hasReached10Killstreak = Boolean.parseBoolean(token2);
				...
				case 19:
					if (token.equals("stage")) {
						p.getDoricsQuest().setStage(token3[0], Integer.parseInt(token3[1]));
					}
					break;
				case 20:
					if (token.equals("stage")) {
						p.getMonkeyMadness().setStage(token3[0], Integer.parseInt(token3[1]));
					}
					break;
				case 21:
					if (token.equals("stage")) {
						p.getDragonSlayer2().setStage(token3[0], Integer.parseInt(token3[1]));
					}
					break;	
				case 22:
					if (token.equals("stage")) {
						p.getMurderMystery().setStage(token3[0], Integer.parseInt(token3[1]));
					}
					break;
				case 23:
					if (token.equals("stage")) {
						p.getCooksAssistant().setStage(token3[0], Integer.parseInt(token3[1]));
					}
					break;
				}
			} else {
	...
	public static boolean saveGame(Player p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("pkrestricted = " + p.PK_RESTRICTED);
			characterfile.newLine();
			characterfile.write("claimedpure = " + p.claimedPurePackage);
			characterfile.newLine();
			...
			characterfile.newLine();
			for (Entry<String, CollectionLog> log : p.collectionLog.minigame_logs.entrySet()) { 

				characterfile.write("kill-log-amount = " + log.getKey() + "\t");
				characterfile.write(Integer.toString(log.getValue().amount)
						, 0, Integer.toString(log.getValue().amount).length());
				characterfile.newLine();

				characterfile.write("kill-log-inventory = " + log.getKey() + "\t");

				String inventory = ""; 
				for (Entry<Integer, Integer> loot : log.getValue().loots.entrySet()) { 
					int item = loot.getKey();
					if (item < 0)
						continue;
					int amount = loot.getValue();
					if (amount < 0)
						amount = 0;
					inventory += item+"\t"+amount + "\t";
				}  
				inventory = inventory.substring(0, inventory.length()-2);
				characterfile.write(inventory);
				characterfile.newLine();
			}
 
			for (Entry<String, CollectionLog> log : p.collectionLog.minigame_logs.entrySet()) { 

				characterfile.write("mini-log-amount = " + log.getKey() + "\t");
				characterfile.write(Integer.toString(log.getValue().amount)
						, 0, Integer.toString(log.getValue().amount).length());
				characterfile.newLine();

				characterfile.write("mini-log-inventory = " + log.getKey() + "\t");
			...
			for (Entry<String, CollectionLog> log : p.collectionLog.clue_logs.entrySet()) {
				characterfile.write("clue-log-amount = " + log.getKey() + "\t");
				characterfile.write(Integer.toString(log.getValue().amount)
						, 0, Integer.toString(log.getValue().amount).length());
				characterfile.newLine();

				characterfile.write("clue-log-inventory = " + log.getKey() + "\t");
			...
			for (Entry<String, CollectionLog> log : p.collectionLog.other_logs.entrySet()) {

				characterfile.write("other-log-amount = " + log.getKey() + "\t");
				characterfile.write(Integer.toString(log.getValue().amount)
						, 0, Integer.toString(log.getValue().amount).length());
				characterfile.newLine();

				characterfile.write("other-log-inventory = " + log.getKey() + "\t");
			...
			characterfile.newLine();
			characterfile.write("privatebounty = " + p.privateBounty, 0, 12); 
			characterfile.newLine();
			characterfile.write("hasreached5killstreak = " + p.hasReached5Killstreak, 0, 12); 
			characterfile.newLine();
			characterfile.write("hasreached6killstreak = " + p.hasReached6Killstreak, 0, 12); 
			characterfile.newLine();
			characterfile.write("hasreached7killstreak = " + p.hasReached7Killstreak, 0, 12); 
			characterfile.newLine();
			characterfile.write("hasreached8killstreak = " + p.hasReached8Killstreak, 0, 12); 
			characterfile.newLine();
			characterfile.write("hasreached9killstreak = " + p.hasReached9Killstreak, 0, 12); 
			characterfile.newLine();
			characterfile.write("hasreached10killstreak = " + p.hasReached10Killstreak, 0, 12); 
			characterfile.newLine();
			...
			for (int idx = 0; idx < 6; idx++) {
				PresetClass preset = p.presetManager.presets[idx];
				characterfile.write("preset-title = " + idx + "\t", 0, 17);
				characterfile.write(preset.title, 0, preset.title.length());
				characterfile.newLine();

				characterfile.write("preset-spellbook = " + idx + "\t", 0, 21);
				characterfile.write(preset.spellbook, 0, preset.spellbook.length());
				characterfile.newLine();
				

				characterfile.write("preset-inventory = " + idx + "\t", 0, 21);

				String inventory = "";
				for (int inv = 0; inv < preset.inventory.length; inv++) {
					int item = preset.inventory[inv], amount = preset.inventoryN[inv];
					if (item < 0)
						item = 0;
					if (amount < 0)
						amount = 0;
				...
				characterfile.write("preset-equipment = " + idx + "\t", 0, 21);

				String equipment = "";
				for (int equip = 0; equip < preset.equipment.length; equip++) {
					int item = preset.equipment[equip], amount = preset.equipmentN[equip];
					if (item < 0)
						item = 0;
					if (amount < 0)
						amount = 0;
				...
				characterfile.write("preset-prayers = " + idx + "\t", 0, 19);
				
				String prayers = "";
				for (int pray = 0; pray < preset.quickprayers.length; pray++)
					prayers += preset.quickprayers[pray] + "\t";
				prayers = prayers.substring(0, prayers.length()-2);
				characterfile.write(prayers);
				characterfile.newLine(); 
			}
