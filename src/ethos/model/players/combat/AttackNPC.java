...
import ethos.model.npcs.bosses.kraken.impl.SpawnEntity;
	...
	public static void calculateCombatDamage(Player attacker, NPC defender, CombatType combatType, Special special) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if(damage2 > -1 && attacker.getItems().isWearingItem(22325)) {
					damage3 = damage3/3;
				}
				...
				case 21012: //Dragon Hunter crossbow
					if (attacker.lastWeaponUsed == 21012 && defender.isDemon()) {
						damage *= 1.2;
						accuracy *= 1.2;
					}
					break;
				...
				if (attacker.magicDef) {
					if (defender.npcType != 7413) {
						attacker.getPA().addSkillXP((damage * (attacker.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE) / 3), 1, true);
...
//				break;
		...
		for (int j = 0; j < NPCHandler.npcs.length; j++) {
			if (NPCHandler.npcs[j] != null && NPCHandler.npcs[j].getHealth().getMaximum() > 0) {
				...
				if ((nX - pX == -1 || nX - pX == 0 || nX - pX == 1) && (nY - pY == -1 || nY - pY == 0 || nY - pY == 1)) {
					if (NPCHandler.npcs[j].heightLevel == NPCHandler.npcs[i].heightLevel) {
	...
	public static void attackNpc(Player c, int i) {
		...
		if (c.playerEquipment[c.playerWeapon] == 22323 && c.playerLevel[6] >= 1 && c.spellId == 0 && !c.autocasting) {
			c.autocasting = true;
			c.spellId = 99;
			c.autocastId = 99;
		}

		/* volatile nightmare staff */ /* remove after adding special bar */
		...
		if (c.playerEquipment[c.playerWeapon] == 20733 && c.playerLevel[6] >= 1 && c.spellId == 0 && !c.autocasting) {
			c.autocasting = true;
			c.spellId = 102;
			c.autocastId = 102;
		}
...
//				c.getPA().refreshSkill(5);	
		...
		if(npc.npcType == 493 || npc.npcType == 496) {
			if (npc.npcType == 496 && c.krakenTent <= 3) {
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if (c.attackTimer <= 0) {
				...
				case 22550:
					...
					break;
				case 22545:
					c.setVigCharge(c.getVigCharge() - 1);
					if (c.getVigCharge() <= 0) {
						c.setVigCharge(0);
						c.sendMessage("@pur@Your Mace has run out of charges");
						c.getItems().deleteItem(22545, 1, c.playerWeapon);
						c.getItems().wearItem(22542, 1, c.playerWeapon);
						//c.getItems().addItemUnderAnyCircumstance(22542, 1);
				}
					break;
				case 22555:
					c.setSceptreCharge(c.getSceptreCharge() - 1);
					if (c.getSceptreCharge() <= 0) {
						c.setSceptreCharge(0);
						c.sendMessage("@pur@Your Sceptre has run out of charges");
						c.getItems().deleteItem(22555, 1, c.playerWeapon);
						c.getItems().wearItem(22552, 1, c.playerWeapon);
						//c.getItems().addItemUnderAnyCircumstance(22552, 1);	
				}
					break;
					...
					case 22323:
						if (c.autocasting) {
							...
							if (passive < 6) {
								...
								c.spellId = 101;
							c.getHealth().increase((c.getHealth().getAmount() / 9));
							if (c.debugMessage)
								c.sendMessage("@red@Healed by" + (c.getHealth().getAmount() / 9) +"");
							}
						}
						break;
