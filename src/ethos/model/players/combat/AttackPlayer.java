...
public class AttackPlayer {
	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		default:
		case MELEE:
			experience += (int) Math.ceil((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE))) / 3;
			skills.add(Skill.HITPOINTS.getId());
			c.getPA().addSkillXP((int) Math.ceil((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE))) / 3, 3, false);

			switch (c.fightMode) {
			default:
			case 0: // Accurate
				skills.add(Skill.ATTACK.getId());
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE)), 0, false);
				break;
			case 1: // Block
				skills.add(Skill.DEFENCE.getId());
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE)), 1, false);
				break;
			case 2: // Aggressive
				skills.add(Skill.STRENGTH.getId());
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE)), 2, false);
				break;
			case 3: // Controlled
				int split = (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE)) / 3;
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MELEE_EXP_RATE));
				for (int i = 0; i < 3; i++) {
		...
		case RANGE:
			skills.add(Skill.HITPOINTS.getId());
			experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE)) / 3;
			c.getPA().addSkillXP((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE)) / 3, 3, false);
			switch (c.fightMode) {
			case 0: // Accurate
			case 2: // Rapid
				skills.add(Skill.RANGED.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE));
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE), 4, false);
				break;
			case 1: // Block
			case 3:
				skills.add(Skill.RANGED.getId());
				skills.add(Skill.DEFENCE.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE)) + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE) / 2);
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE), 1, false);
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.RANGE_EXP_RATE) / 2, 4, false);
				break;
			}
			break;
		case MAGE:
			...
			experience += ((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MAGIC_EXP_RATE)) / 3) + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE));
			c.getPA().addSkillXP((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MAGIC_EXP_RATE)) / 3, 3, false);
			c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 8 : Config.MAGIC_EXP_RATE), 6, false);
			break;
		}
		if (experience > 0) {
	...
	public static void playerDelayedHit(final Player c, final int i, final Damage d) {
		if (PlayerHandler.players[i] != null) {
			...
			if (c.playerEquipment[c.playerWeapon] == 24425 && c.playerLevel[6] >= 1 && c.spellId == 0 && !c.autocasting) {
			...
			if (c.usingSpecial) {
				...
				c.playerLevel[5] += damageDealth;
				c.getPA().refreshSkill(5);
				
//				c.sendMessage("@blu@Restoring " + damageDealth + " @blu@prayer");
//				c.playerLevel[5] += damage.getAmount() / 2;
//				c.getPA().refreshSkill(5);	
			}
	     	}
		
		
			if (c.playerEquipment[c.playerWeapon] == 24424 && c.playerLevel[6] >= 1 && c.spellId == 0 && !c.autocasting) {
			if (c.usingSpecial) {
				// c.autocasting = false;
				c.spellId = 100;
				c.usingSpecial = false;
	...
	public static void calculateCombatDamage(Player attacker, Player defender, CombatType combatType, Special special) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (attacker.magicDef) {
					attacker.getPA().addSkillXP((damage * (attacker.getMode().getType().equals(ModeType.OSRS) ? 12 : Config.MELEE_EXP_RATE) / 3), 1, false);
		...
		if (damage2 > -1) {
			...
			if(weapon == 22325) {
				attacker.getDamageQueue().add(new Damage(defender, damage2/2, delay, attacker.playerEquipment, hitmark2, combatType, special));
			}
		}
		delay += 1;
	}
	...
	public static void attackPlayer(Player c, int i) {
		...
		if (c.playerEquipment[c.playerWeapon] == 11907 || c.playerEquipment[c.playerWeapon] == 12899 || c.playerEquipment[c.playerWeapon] == 22516) {
			...
			if (PlayerHandler.players[i] != null) {
				...
				if (c.attackTimer <= 0) {
					...
					if (!c.getCombat().correctBowAndArrows() && Config.CORRECT_ARROWS && c.usingBow && !c.getCombat().usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 4734
							&& c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 11785 && c.playerEquipment[c.playerWeapon] != 21012 && c.playerEquipment[c.playerWeapon] != 21902 && !c.usingMagic && !c.getItems().isWearingItem(12926) && c.playerEquipment[c.playerWeapon] != 19478 && c.playerEquipment[c.playerWeapon] != 19481) {
					...
					if (c.playerEquipment[c.playerWeapon] == 9185 && !c.getCombat().properBolts() && !c.usingMagic
							|| c.playerEquipment[c.playerWeapon] == 11785 && !c.getCombat().properBolts() && !c.usingMagic || c.playerEquipment[c.playerWeapon] == 21012 && !c.getCombat().properBolts() && !c.usingMagic || c.playerEquipment[c.playerWeapon] == 21902 && !c.getCombat().properBolts() && !c.usingMagic) {
