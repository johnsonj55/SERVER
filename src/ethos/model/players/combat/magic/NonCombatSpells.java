...
public class NonCombatSpells extends MagicRequirements {
	...
	public static void bonesToBase(Player c, int levelReq, int[] runes, int[] amount, int[] item) {
		...
		c.getPA().addSkillXP(c.getMode().isOsrs() ? 140 : 214, 6, true);
		...
		if (System.currentTimeMillis() - c.alchDelay > 700) {
			...
			for (int i = 0; i < data.length; i++) {
				if (itemID == data[i][0]) {
					...
					c.getPA().addSkillXP(data[i][5] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.SMITHING_EXPERIENCE), Player.playerSmithing, true);
					c.getPA().addSkillXP(53 * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE), 6, true);
			...
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				if (c.getItems().playerHasItem(itemId, slot, 1)) {
					...
					if (c.getItems().playerHasItem(itemId, slot, 1)) {
						...
						c.alchDelay = System.currentTimeMillis();
						c.getPA().sendFrame106(6);
						c.getPA().addSkillXP(MagicData.MAGIC_SPELLS[49][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE), 6, true);
			...
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				if (c.getItems().playerHasItem(itemId, slot, 1)) {
					...
					if (c.getItems().playerHasItem(itemId, slot, 1)) {
						...
						c.alchDelay = System.currentTimeMillis();
						c.getPA().sendFrame106(6);
						c.getPA().addSkillXP(MagicData.MAGIC_SPELLS[50][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE), 6, true);
						c.getPA().refreshSkill(6);
					}
				}
			}
