...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {
		...
		case 99999:
			if (c.PK_RESTRICTED == true) {
				c.getPA().closeAllWindows();
				return;
			}
			c.PK_RESTRICTED = false;
			c.getTutorial().autoComplete();
			c.nextChat = -1;
			break;
			case 450:
			...
			case 9052:
				...
				c.sendMessage("@pur@You break the Bracelet down into Ether@pur@");
				c.getPA().closeAllWindows();
				}
				break;
			...
			case 1001:
				...
				if (c.getItems().playerHasItem(3467)) {
		...
		case 156:
			...
			break;
		case 1504://Cooks Assistant
			c.getDH().sendDialogues(1505, 4626);
		break;
		case 1303://Murder Mystery Guard
			c.getDH().sendDialogues(1304, 4218);
		break;
		...
		case 1391:
			c.getDH().sendDialogues(1545, c.npcType);
			break;
		...
		case 5439:
			if (c.dialogueAction == 128) {
				c.getDH().sendDialogues(633, 2461);
				return;
			}
				else; {
	...
	public static void handleOption2(Player c) {
		...
		case 903:
			...
			case 99999:
				c.getPA().closeAllWindows();
				c.PK_RESTRICTED = true; 
				c.nextChat = -1;
				break;
		case 1504://Cooks Assistant
				c.getDH().sendDialogues(1516, 4626);
			break;
		case 1318://Murder Mystery
				c.getDH().sendDialogues(1322, 4220);
			break;
		case 901:
