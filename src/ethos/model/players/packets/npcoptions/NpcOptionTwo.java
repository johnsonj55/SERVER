...
public class NpcOptionTwo {
	...
	public static void handleOption(Player player, int npcType) {
		...
		case 3077:
			...
			player.getDH().sendNpcChat1("You've been playing Mistex for " + time, 3077, "Hans");
		...
		case 6797: // Nieve
			...
			break;
		case 8623: // Konar quo Maten
			if (player.playerLevel[18] < 95) {
				player.getDH().sendNpcChat1("You must have a slayer level of atleast 95 weakling.", 8623, "Konar quo Maten");
				return;
			} else {
				player.getDH().sendDialogues(3304, player.npcType);
			}
		...
		case 3106:
			...
			break;
		case 3108:
			player.getThieving().steal(Pickpocket.Knight_Of_Ardougne, NPCHandler.npcs[player.rememberNpcIndex]);
			break;
		case 637:
