...
import ethos.model.content.BrimstoneChest;
...
import ethos.model.content.group_ironman.IronmanTeamInterface;
...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		switch (objectType) {
		case 34662:
			BrimstoneChest.searchChest(c);
		break;
		case 3192:
			IronmanTeamInterface.openInterface(c);
		break;

		//Mount Karuulm Objects
		case 34359://Entrance
			...
			public void execute(CycleEventContainer container) {
			    ...
			    switch (ticks++) {
			    case 0:
			    	c.getPA().playerWalk(1311, 3807);
			        c.turnPlayerTo(1310, 3807);
			    break;
			    case 2:
			    	AgilityHandler.delayEmote(c, "PULL_LEVER", 1311, 10188, 0, 2);
			    	container.stop();
			    break;
			    }
			}
		...
		case 34514://Exit Cave
			AgilityHandler.delayEmote(c, 844, 1311, 3807, 0, 1);
			c.sendMessage("You safely exit the cave.");
		break;
		case 34530://Stairs Going Up
			if (c.heightLevel == 0) {//First level
				AgilityHandler.delayEmote(c, 828, 1334, 10205, 1, 1);
			} else if (c.heightLevel == 1) {//Second Level
				AgilityHandler.delayEmote(c, 828, 1318, 10189, 2, 1);
			}
		break;
		case 34531://Stairs Going Down
			if (c.heightLevel == 2) {//Third Level
				AgilityHandler.delayEmote(c, 828, 1313, 10189, 1, 1);
			} else if (c.heightLevel == 1) {//Second Level
				AgilityHandler.delayEmote(c, 828, 1329, 10205, 0, 1);
			}
		break;
		case 34544://Climb over rock barriers
			if (c.absX == 1320) {
				AgilityHandler.delayEmote(c, 840, 1322, 10205, 0, 1);
			} else if (c.absX == 1322) {
				AgilityHandler.delayEmote(c, 840, 1320, 10205, 0, 1);
			} else if (c.absX == 1303) {
				AgilityHandler.delayEmote(c, 840, 1301, 10205, 0, 1);
			} else if (c.absX == 1301) {
				AgilityHandler.delayEmote(c, 840, 1303, 10205, 0, 1);
			} else if (c.absY == 10214) {
				AgilityHandler.delayEmote(c, 840, 1311, 10216, 0, 1);
			} else if (c.absY == 10216) {
				AgilityHandler.delayEmote(c, 840, 1311, 10214, 0, 1);
			}
		break;
		case 30397:
			if (Config.SPAWNED_BOSS == true) {
				if (c.getX() == 3109) {
					c.getPA().movePlayerUnconditionally(3110, c.getY(), c.getHeight());
					
				} else if (c.getX() == 3110) {
					c.getPA().movePlayerUnconditionally(3109, c.getY(), c.getHeight());
				}
			} else {
				c.sendMessage("There must be $100 in donations within the 12 hours to reach this.");
			}
			break;
		case 34548://Climb rock barrier to alchemical hydra area
			if (c.absY == 10250) {
				AgilityHandler.delayEmote(c, 840, 1351, 10252, 0, 1);
			} else if (c.absX == 10252) {
				AgilityHandler.delayEmote(c, 840, 1351, 10250, 0, 1);
			}
		break;
		//End mount karuulm objects
		case 33455:
			...
			c.sendMessage("@blu@You pray to the statue and recover health, prayer, run energy and special attack.");
		break;
			case 34553:
				/*
			case 34554:
				if (Boundary.isIn(c, AlchemicalHydraConstants.AREA)) {
					c.getPA().sendMessage("The door seems to have locked unexpectedly!");
//				} else if (!c.getSlayer().getTask().isPresent()) {
//					c.getDH().sendStatement("You currently do not have an assignment, ", "you may receive one by talking to a Slayer Master.");
//				} else c.getSlayer().getTask().ifPresent(task -> {
//					if (task.getPrimaryName().contains("hydra")) {
//					c.createAlchemicalHydraInstance();
//					} else {
//						c.getDH().sendStatement("Your current assignment does not correspond to the one ", "required to enter this room.");
//					}
				}/*)
...
//		switch (objectType) { // i off this
		 ...
		 */
			case 8720:
				...
				c.sendMessage("@red@ Thank you for supporting Mistex!");
				break;
			case 31556:
		...
		case 20877:
			...
			break;
		case 30258:
			AgilityHandler.delayFade(c, "CLIMB_DOWN", 1457, 10096, 1, "You step inside the gibbet.",
					"The chain creaks as you descend further into the Chasm.", 3);
			c.getDiaryManager().getKaramjaDiary().progress(KaramjaDiaryEntry.ENTER_BRIMHAVEN_DUNGEON);
			break;
		case 16664:
			AgilityHandler.delayFade(c, "CLIMB_DOWN", 1457, 10096, 0, "Deep Wilderness Dungeon.",
					"Welcome to Deep Wilderness Dungeon.", 3);
			c.getDiaryManager().getKaramjaDiary().progress(KaramjaDiaryEntry.ENTER_BRIMHAVEN_DUNGEON);
			break;
		case 16665:
			AgilityHandler.delayFade(c, "CLIMB_DOWN", 3054, 3927, 0, "Left Deep Wilderness Dungeon.",
					"You Left Deep Wilderness Dungeon.", 3);
			c.getDiaryManager().getKaramjaDiary().progress(KaramjaDiaryEntry.ENTER_BRIMHAVEN_DUNGEON);
			break;
		case 30259:
			AgilityHandler.delayFade(c, "CLIMB_UP", 1457, 10096, 2, "You step inside the gibbet.",
					"The chain creaks as you descend further into the Chasm.", 3);
			c.getDiaryManager().getKaramjaDiary().progress(KaramjaDiaryEntry.ENTER_BRIMHAVEN_DUNGEON);
			break;
		case 20878:
		...
		case 11836:
			...
			break;
		case 2665://Murder Mystery Gate
			c.getDH().sendDialogues(1312, 4228);
		break;
		
		case 25748:
		case 25750://Murder Mystery front door
			if (c.absY == 3572) {
				AgilityHandler.delayEmote(c, 2140, 2740, 3574, 0, 2);
				Server.itemHandler.createGroundItem(c, 1809, 2747, 3577, 0, 1);
			}
			if (c.absY == 3573) {
				AgilityHandler.delayEmote(c, 2140, 2741, 3571, 0, 2);
			}
		break;
		case 25682:
			if (c.heightLevel == 0) {
				AgilityHandler.delayEmote(c, 2140, 2736, 3580, 1, 2);
			}
		break;
		...
		case 16465:
			...
			break;
		case 30364:// Mining Guild Entrance
			...
			} else if (c.absX == 3046 && c.absY == 9756) {
				c.getPA().movePlayer(3046, 9757, 0);
			}
			break;
		case 1524:// Wildy altar doors
			if (c.absX == 2957 && c.absY == 3821) {
					c.getPA().movePlayer(2958, 3821, 0);
		
			}	 else if (c.absX == 2958 && c.absY == 3821) {
				c.getPA().movePlayer(2957, 3821, 0);
			}
			break;
		case 1521:// Wildy altar doors
			if (c.absX == 2957 && c.absY == 3820) {
					c.getPA().movePlayer(2958, 3820, 0);
		
			}	 else if (c.absX == 2958 && c.absY == 3820) {
		...
		case 1579:
			...
			break;
		case 17385:
		/*	if((obX >= 3020 && obY <= 9739) || (obX >= 3019 && obY <= 9738) || (obX >= 3018 && obY <= 9739) || (obX >= 3019 && obY <= 9742)) { // Mining Guild
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3021, 3338, 0, 2);}*/
			if (obX >= 3097 && obY <= 9867) { // Edge Dungeon Ladder
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3098, 3507, 0, 2);}
			if (obX >= 3116 && obY <= 9852) { // Edge Dungeon hill giant Ladder
				AgilityHandler.delayEmote(c, "CLIMB_UP", 316, 3451, 0, 2);}
			if (obX == 3020 && obY == 9739) { // Mining Guild Exit East
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3021, 3339, 0, 2);}
			if (obX == 3019 && obY == 9740) { // Mining Guild Exit North
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3019, 3341, 0, 2);}
			if (obX == 3018 && obY == 9739) { // Mining Guild Exit West
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3017, 9739, 0, 2);}
			if (obX == 3019 && obY == 9738) { // Mining Guild Exit South
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3019, 3339, 0, 2);}
			else // Taverly Dungeon Ladder
	...
	//		Kraken krak = c.createKrakenInstance();
//			player.getKraken().initialize();
		...
		case 26760:
			...
			if ( c.absX == 3085 && c.absY == 3484 ) {
				c.getDH().sendDialogues(631, -1);
			} else if (c.absX == 3085 && c.absY == 3483 ) {
				c.getPA().movePlayer(3085, 3484, 0);
			}	
			break;
		case 19206:
