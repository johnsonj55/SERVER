...
public class Fishing extends SkillHandler {
	...
	public static int[] isHarpoon = { 311, 21028, };
	/**
	 * Fishing data
	...
	public static int[][] data = { 
			...
			{ 15, 65, 3157, -1, 3142, 105, 620, -1, -1, -1, 30200 }, // Karambwan
			{ 16, 82, 307, 13431, 13439, 118, 622, -1, -1, -1, 30400 }, // Anglerfish
			{ 17, 35, 21028, -1, 359, 80, 618, 371, 50, 100, 32300 }, //Tuna + Swordie with Dharpoon
			{ 18, 76, 21028, -1, 383, 110, 618, -1, -1, -1, 31200 }, // SHARK w DHarpoon
	};
	...
	public static void attemptdata(final Player player, int npcId) {
		...
		for (int i = 0; i < data.length; i++) {
			if (npcId == data[i][0]) {
				...
				double experience = data[i][5] * (player.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.FISHING_EXPERIENCE) * multiplier; // XP
	...
	private static boolean hasFishingEquipment(Player c, int equipment) {
		if (!c.getItems().playerHasItem(equipment)) {
			if (equipment == 311) {
				if (!c.getItems().playerHasItem(311) && !c.getItems().playerHasItem(10129) && c.playerEquipment[3] != 10129 && !c.getItems().playerHasItem(21028) && c.playerEquipment[3] != 21028) {
