...
//import ethos.model.players.skills.slayer.Slayer.SLAYER_HELMETS;
...
public class Slayer {
	...
	public void createNewTask(int masterId) {
		SlayerMaster.get(masterId).ifPresent(m -> {
			...
			List<Task> available = retainObtainable(m.getAvailable());
			task = Optional.of(Misc.randomTypeOfList(available));
			
			taskAmount = !task.equals("tztok-jad") && biggerBossTasks ? Misc.random(Range.between(task.get().getMinimum(), task.get().getMaximum())) + Misc.random(25) + 40 : Misc.random(Range.between(task.get().getMinimum(), task.get().getMaximum()));
			//m.getId() == 6797 && 
			
			player.talkingNpc = m.getId();
			if(task.get().getArea() != null) {
				player.getDH().sendNpcChat3("You are assigned to kill " + taskAmount + " " + task.get().getPrimaryName() + "", "in " + Misc.capitalize(task.get().getArea().name().toLowerCase().replace("_", " ").trim()), "Come talk to me when you finish this task.", player.talkingNpc, "Konar quo Maten");
			} else {
				...
				master.ifPresent(m -> {
					...
					case 7663:
						...
						case 8623:
							...
							: task.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? 3 : Config.SLAYER_EXPERIENCE),
									Skill.SLAYER.getId(), true);
							int npcCombat = npc.getDefinition().getNpcCombat();
							double max = 1000;
							if(npcCombat >= 350) {
								max = 50;
							} else if(npcCombat >= 100) {
								max = -0.2 * npcCombat + 120;
							} else {
								max = 0.2 * Math.pow(npcCombat - 100, 2) + 100;
							}
							double chance = Misc.randomDouble(0, max);
							if(chance <= 1.0) {
								Server.itemHandler.createGroundItem(player, 23083, npc.getX(), npc.getY(), npc.getHeight(), 1);
							}
							break;
					}
					...
					if (taskAmount == 0) {
						...
						DailyTasks.increase(player, PossibleTasks.SLAYER_TASK);
	 ...
	 */
	private List<Task> retainObtainable(Task[] tasks) {
		int pSlayerLevel = player.playerLevel[Skill.SLAYER.getId()];

		List<String> blocked = Arrays.asList(removed).stream().map(String::toLowerCase).collect(Collectors.toList());
		Predicate<Task> allowedTask = task -> {
			if(task.getPrimaryName() == null)
				return false;
			String taskName = task.getPrimaryName().toLowerCase();
			if(taskName.equalsIgnoreCase("cerberus") && !learnedCerberusRoute)
				return false;
			if(blocked.contains(taskName))
				return false;
			if(task.getLevel() > pSlayerLevel)
				return false;
			return true;
		};

		return Stream.of(tasks).filter(allowedTask).collect(Collectors.toList());
	}
	...
	public void cancelTask() {
		int rankPoints = 
		player.getRights().getPrimary() == Right.CONTRIBUTOR ? 0 : 30;
...
//			updatePoints();
		...
		case 160057:
			...
			if (points < 100) {
				player.sendMessage("You need 100 slayer points to extend tasks.");
				return true;
			}
			points -= 100;
			biggerBossTasks = true;
			player.buySlayerTimer = System.currentTimeMillis();
			player.sendMessage("You will now receive extended tasks.");
			updatePoints();
			return true;
