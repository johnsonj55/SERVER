...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 9) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @blu@" + getSpecialItemValue(removeId) + " @bla@ ] Donator Shards.");
			return;
		}
		if (c.myShopId == 18) {
...
/*		case 30600:
		...
		case 12596:
			return 12;
		case 6199:
			//if (c.myShopId == 122) {//i have get this error trying to make the castle wars shop
				//switch (itemId) {
				/*Red Items*/
				case 4071:
					return 4;
...
//			castleSlot = c.getItems().getItemSlot(4067);
			if (TotPrice2 <= 1) {
				TotPrice2 = (int) Math.floor(getItemShopValue(itemID, 0, fromSlot));
				TotPrice2 *= 1.66;
			}
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 12) {
			...
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(32950, itemValue)) {
				c.sendMessage("You do not have enough trading sticks to buy this item.");
				return;
			} else {
				c.getItems().deleteItem(32950, itemValue);
				c.getItems().addItem(itemID, amount);
			...
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(4067, itemValue)) {
				c.sendMessage("You do not have enough CastleWars ticket to buy this item.");
				return;
			} else {
				c.getItems().deleteItem(4067, itemValue);
				c.getItems().addItem(itemID, amount);
				c.amDonated += itemValue;
				c.updateRank();
				c.getItems().resetItems(3823);
			...
			if (c.pcPoints >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					...
					log.name = "pest control"; 
					log.addLoot(new GameItem(itemID, amount));
					c.collectionLog.addLog(log, CollectionLogType.MINIGAMES);
